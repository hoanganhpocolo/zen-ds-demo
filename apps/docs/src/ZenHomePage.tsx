...
function UspCard({ className, icon, title, subtitle, children }: {
  ...
  return (
    <div className={`zen-home-usp-card ${className ?? ''}`}>
      <div className="zen-home-usp-card-header">
        <img className="zen-home-usp-card-icon" src={`/home/${icon}`} alt="" />
        <div className="zen-home-usp-card-text">
          <h3 className="text-h3">{title}</h3>
          <p className="text-body-base zen-home-usp-card-subtitle">{subtitle}</p>
        </div>
      </div>
      <div className="zen-home-usp-card-slot">
        {children}
      </div>
    </div>
  );
...
export function ZenHomePage() {
  const navigate = useNavigate();
  const pageRef = useRef<HTMLDivElement>(null);       // root for fonts-ready fade
  ...
  // Fade the whole page in once BT Solisiert has loaded, so the user
  // never sees a fallback → display-font swap.
  useEffect(() => {
    const el = pageRef.current;
    if (!el) return;
    let cancelled = false;
    const reveal = () => { if (!cancelled) el.classList.add('is-fonts-ready'); };
    const fontsReady = document.fonts?.ready ?? Promise.resolve();
    const cap = new Promise<void>((r) => window.setTimeout(r, 2000));
    Promise.race([fontsReady, cap]).then(reveal);
  ...
  // Disable browser scroll restoration so reloads always start at top
  // (avoids a flash where the page appears mid-scroll then snaps).
  useEffect(() => {
  ...
  // Wait for fonts to load before starting the sequence so the title
  // never flashes from fallback → BT Solisiert mid-animation.
  useEffect(() => {
    ...
    // Font-gated start. Cap the wait at 2s so a stalled network can't
    // block reveal forever — after the cap we start anyway; swap will be
    // invisible because the text is still below the curtain (translateY 110%).
    const fontsReady = document.fonts?.ready ?? Promise.resolve();
    const cap = new Promise<void>((r) => window.setTimeout(r, 2000));
    Promise.race([fontsReady, cap]).then(start);
  ...
  useEffect(() => {
    ...
    const splitIntoLines = () => splitTextIntoLines(titleEl, ABOUT_TITLE, 'zen-home-about-title-line', 'zen-home-about-title-line-inner');
  ...
  useEffect(() => {
    ...
    const doSplit = () => requestAnimationFrame(() => {
      for (const { el } of entries) {
        if (!el) continue;
        splitTextIntoLines(el, el.textContent ?? '', 'zen-home-section-title-line', 'zen-home-section-title-line-inner');
      }
    });
  ...
  return (
    <div ref={pageRef} className="zen-home-page is-js">
      <section ref={heroSectionRef} className="zen-home-hero">
        {/* Bronze drum decorative pattern behind content */}
        <div className="zen-home-hero-pattern" aria-hidden="true">
          <img className="zen-home-hero-pattern-img" src="/home/vnbg.svg" alt="" />
        </div>

        {/* Top navigation */}
        <nav className="zen-home-topbar" aria-label="Primary">
          <div className="zen-home-topbar-left">
            <img className="zen-home-topbar-mark" src="/zen-mark.svg" alt="Zen" width={32} height={30} />
            <span className="zen-home-topbar-brand">ZEN</span>
            <span className="zen-home-topbar-badge">Kaiz 1.0</span>
          </div>
          <div className="zen-home-topbar-right">
            <a href="#icons" className="zen-home-topbar-link">Zen Icon</a>
            <a href="#components" className="zen-home-topbar-link">Components</a>
            <a href="#demo" className="zen-home-topbar-link">Demo</a>
            <a href="#pricing" className="zen-home-topbar-link">Pricing</a>
            <Button variant="accent" size="m" icon={<Figma size={20} />}>Download Figma</Button>
          </div>
        </nav>

        {/* Hero content grid — title | logo | copy */}
        <div className="zen-home-hero-grid">
          <h1 className="zen-home-hero-title">
            <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 0 }}>Zen</span></span>
            <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 1 }}>Design</span></span>
            <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 2 }}>System</span></span>
          </h1>


          <div className="zen-home-hero-right">
            <div className="zen-home-hero-subtitle text-subheading">
              <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 0 }}>We bring you the ultimate Figma UI kit and React component library.</span></span>
              <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 1 }}>Empowers you to build beautiful and user friendly interfaces quickly and efficiently.</span></span>
            </div>
            <div className="zen-home-hero-meta text-capsline-s">
              <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 0 }}>ĐÌ ZAI STUDIO</span></span>
              <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 1 }}>ZEN KAIZ</span></span>
              <span className="zen-home-hero-title-line"><span className="zen-home-hero-title-line-inner" style={{ ['--line-index' as string]: 2 }}>2026</span></span>
            </div>
          </div>
        </div>

        {/* Ink landscape painting — WebGL liquid distortion following mouse */}
        <HeroLandscape />
      </section>

      {/* ── Section 2 — About UIAI ── */}
      <section ref={aboutRef} className="zen-home-about">
        {/* Sunray — inside about section, z-index between video(1) and content(3) */}
        <div ref={heroRef} className="zen-home-about-sunray" aria-hidden="true">
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 0 }} src="/home/Sunray01.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 1 }} src="/home/Sunray02.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 2 }} src="/home/Sunray03.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 3 }} src="/home/Sunray04.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 4 }} src="/home/Sunray05.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 5 }} src="/home/Sunray06.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 6 }} src="/home/Sunray07.png" alt="" />
          <img className="zen-home-hero-sun-layer" style={{ ['--layer-index' as string]: 7 }} src="/home/Sunray08.png" alt="" />
        </div>

        {/* Video background */}
        <div className="zen-home-about-video-wrap">
          <video
            className="zen-home-about-video"
            autoPlay
            muted
            loop
            playsInline
          ...
          <div className="zen-home-about-noise" aria-hidden="true" />
        </div>

        {/* Content block */}
        <div className="zen-home-about-content">
          <div className="zen-home-about-text">
            <p className="text-body-extra zen-home-about-label">
              About UIAI &lt;3
            </p>
            <h2 ref={titleRef} className="text-display-3 zen-home-about-title">
              {ABOUT_TITLE}
            </h2>
          </div>
          <div className="zen-home-about-cta">
      ...
      <section className="zen-home-showcase">
        {/* Noise pattern overlay */}
        <div className="zen-home-showcase-noise" aria-hidden="true" />

        {/* Mockup image */}
        <div className="zen-home-showcase-mockup">
          <img src="/home/mokup.webp" alt="Component showcase" />
        </div>

        {/* Text block */}
        <div ref={showcaseRef} className="zen-home-showcase-text">
          <p className="text-body-extra zen-home-showcase-label">
            Why You Should Care
          </p>
          <h2 ref={showcaseTitleRef} className="text-display-3">The ultimate Design System</h2>
          <p className="text-subheading zen-home-showcase-subtitle">
        ...
        {/* USP Cards Grid */}
        <div ref={cardGridRef} className="zen-home-usp-grid">
          {/* Row 1: 2 cards (6+6) */}
          <UspCard className="zen-home-usp-card-6" icon="Type=Dark theme.png" title="Dark theme" subtitle="Your retinas called — they want a break. Just turn off the light. It smooth, easy-on-the-eyes UI that looks stunning at 2 AM.">
            <DarkThemeSlot />
          </UspCard>
          <UspCard className="zen-home-usp-card-6" icon="Type=Token.png" title="4-Tier Token Architecture" subtitle="Change one value, watch it ripple across your entire system like magic. No more &quot;wait, which blue is this?&quot; moments.">
            <img src="/home/USP Token.webp" alt="Token architecture" />
          </UspCard>

          {/* Row 2: 2 cards (5+7) */}
          <UspCard className="zen-home-usp-card-5" icon="Type=Component 2.png" title="2000+ variants" subtitle="Every component comes with states, sizes, and variants so you never start from scratch again.">
            <img src="/home/USP Components.webp" alt="Component variants" />
          </UspCard>
          <UspCard className="zen-home-usp-card-7" icon="Type=Customize.png" title="Customize your milk tea" subtitle="Swap themes like swap your ex. Light, dark, rounded, sharp — pick a vibe and IUAI adapts instantly. Your brand, your rules. We just make it ridiculously easy.">
            <CustomizeSlot />
          </UspCard>

          {/* Row 3: 3 cards (4+4+4) */}
          <UspCard className="zen-home-usp-card-4" icon="Type=Figma.png" title="Figma ↔ Code 1:1" subtitle="Because it coded by the designer">
...
<span class="cl-ln">10</span> &lt;/<span class="cl-fn">Button</span>&gt;` }} />
          ...
          <UspCard className="zen-home-usp-card-4" icon="Type=Icon.png" title="1,700 Icons — 3 Styles" subtitle="To much Redbull to get this shit done">
            <IconsSlot />
          </UspCard>
          <UspCard className="zen-home-usp-card-4" icon="Type=Plugin.png" title="Plugin included" subtitle="In the future, but we promise">
      ...
      <section className="zen-home-contributors">
        <div ref={contributorsRef} className="zen-home-contributors-content">
          <p className="text-body-extra zen-home-contributors-label">Behind the AI</p>
          <h2 ref={contributorsTitleRef} className="text-display-2">Human Contributors</h2>
          <p className="text-subheading zen-home-contributors-subtitle">
        ...
        <div className="zen-home-contributors-grid">
          {[
            ...
            <div key={person.handle} className="zen-home-contributor">
              <div className="zen-home-contributor-frame">
                <img className="zen-home-contributor-frame-img" src={`/home/${person.frame}`} alt="" />
                <div className="zen-home-contributor-avatar">
                  <img src={`/home/${person.avatar}`} alt={person.name} />
                </div>
              </div>
              <h3 className="text-h3">{person.name}</h3>
              <p className="text-body-extra zen-home-contributor-handle">{person.handle}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <footer ref={footerRef} className="zen-home-footer">
        <img className="zen-home-footer-bg" src="/home/Footer back.webp" alt="" aria-hidden="true" />
        <div className="zen-home-footer-sunray" aria-hidden="true">
          <img src="/home/Sunray01.png" alt="" />
          <img src="/home/Sunray02.png" alt="" />
          <img src="/home/Sunray03.png" alt="" />
          <img src="/home/Sunray04.png" alt="" />
          <img src="/home/Sunray05.png" alt="" />
          <img src="/home/Sunray06.png" alt="" />
          <img src="/home/Sunray07.png" alt="" />
          <img src="/home/Sunray08.png" alt="" />
        </div>
        <img className="zen-home-footer-front" src="/home/Footer Front.webp" alt="" aria-hidden="true" />
      </footer>
    </div>
  );
}
