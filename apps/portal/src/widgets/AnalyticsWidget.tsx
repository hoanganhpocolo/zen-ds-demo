      ...
      <div className="wc-analytics-body">
        ...
        <div className="wc-metrics-row">
          {metrics.map((m, i) => (
            <div key={i} className="wc-metric-mini">
              <p className="text-body-small wc-metric-title">{m.title}</p>
              <p className="wc-metric-value">{m.value}</p>
              <div className="text-caption wc-metric-change">
        ...
        <div className="wc-charts-row">
          ...
          <div className="wc-chart-card">
            <p className="text-body-small wc-chart-title">Distribution By Shop</p>
            <div className="wc-chart-area">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v) => `${(Number(v) / 1000).toFixed(0)}K`} contentStyle={tooltipStyle} itemStyle={{ fontSize: 12, padding: 0 }} />
                </PieChart>
              </ResponsiveContainer>
              <div className="wc-pie-center">
                <p className="text-caption wc-pie-total-label">Total</p>
                <p className="text-body-small wc-pie-total-value">362,600k</p>
              </div>
            </div>
            <div className="wc-chart-legend">
              {pieData.map((d, i) => (
                <div key={d.name} className="wc-legend-item">
                  ...
                  <span className="text-caption">{d.name} ({((d.value / 362600) * 100).toFixed(1)}%)</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Bar */}
          <div className="wc-chart-card">
            <p className="text-body-small wc-chart-title">Weekly Gross Booking</p>
            <div className="wc-chart-area">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barData} barGap={1} barSize={14}>
                  ...
                  <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'var(--color-content-neutral-tertiary)' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: 'var(--color-content-neutral-tertiary)' }} axisLine={false} tickLine={false} width={30} />
                  <Tooltip contentStyle={tooltipStyle} labelStyle={{ fontSize: 12, fontWeight: 600, marginBottom: 4 }} itemStyle={{ fontSize: 12, padding: 0 }} cursor={{ fill: 'var(--color-bg-fill-neutral-subtle-light-default)' }} />
            ...
            <div className="wc-chart-legend">
              <div className="wc-legend-item">
                ...
                <span className="text-caption">Gross Booking 10/2025</span>
              </div>
              <div className="wc-legend-item">
                <span className="wc-legend-dot wc-legend-dot-info" />
                <span className="text-caption">Gross Booking 2025-10-24</span>
              </div>
            </div>
          </div>
        </div>
